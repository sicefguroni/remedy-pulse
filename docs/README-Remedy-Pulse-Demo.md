...
Double-click the file `index.html`. It opens in your web browser (Chrome, Safari, etc.) — no installation and no login required. It also works fully offline: the page falls back to built-in system fonts if it can't reach the internet, so it stays readable and functional, just with slightly different typography than intended. When you are online, it additionally pulls its intended Google Fonts (Space Grotesk, Inter, IBM Plex Mono) for the polished look shown in this guide.
