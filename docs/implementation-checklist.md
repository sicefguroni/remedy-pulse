...
**Generated:** 2026-09-03 · fresh run · **updated 2026-09-12**
**Sources:** `remedy-pulse-prd.md`, `remedy-pulse-roadmap.md`, `index.html`, `backend/`, `docs/Remedy Pulse_Reddit Data Access_Use Case.pdf`
**Progress:** 74 / 86 (up from 64/79 — corrected 2026-09-12: the prior update's summary line said 0.24 was "flagged, left open," but its own status note below already recorded it Done; this line now matches the actual checkbox state. Now includes 0.26 (the `index.html` rename) and 0.27 (a fresh sweep's date-validation fix).)

**Changelog (2026-09-12):** Corrected this summary block, which had fallen out of sync with the actual item statuses below (0.24 was recorded Done in its own entry on 2026-09-11 but this line still said "left open" — a documentation bug in the checklist about itself, now fixed). Also closed **0.25 (NEW)**, raised directly by the user ("remove all mock data on logged in sessions"): four places still rendered fabricated content in a real, authenticated session — the Competitors tab's Top Voices/Category Watch cards, the assign-roster dropdown's fallback, and `GET /api/overview`'s `aiSummaryText` (a canned string returned on every single call, regardless of real vs. fake data, since every caller of that endpoint is authenticated by construction). See 0.25's own status note for what changed in each of the four. 342 tests pass, ruff clean, verified with a real-browser check simulating a live session.

**Changelog (2026-09-11):** Re-ran the test suite as ground truth (313 passed, 1 skipped, matching the document's existing claim; `ruff check` clean) before touching anything, confirming the rest of this document was accurate before making changes. Closed three items, all zero-payment engineering work (see each item's own status note for specifics): **8.8** (Competitors/Reviews alias tooltips — also caught and fixed one real regression in `exports.py`'s CSV writer), **7.4** and **8.5** together (the reviews reply-flow wiring gap both items pointed at — a new `POST /api/reviews/by-venue/{venue}/reply` endpoint plus the mockup's modal wired to it). While wiring that, found and closed **0.21 (NEW)**: a `SystemExit` inside two shared fetch functions (Google reviews token loading, GNews article search) could crash the entire scheduler process on a routine failure. Advanced **5.6** (secrets at rest) from not-started to partially done — `GOOGLE_TOKEN_JSON` as a production alternative to a bare `token.json` file. A dedicated second sweep specifically for more bugs of 0.21's shape then found and closed **0.22 (NEW)**: a Meta capability's unexpected fetch failure got no ledger row at all, not even an error one — and surfaced **0.23 (NEW)**: classification is fully implemented but nothing in production ever actually calls it, so real ingested mentions never get a sentiment. Built and closed **0.23** in a follow-up pass: a new `app/jobs/classification_job.py`, registered with a deliberately short 15-minute cadence tied to the PRD's 4-business-hour alert metric, verified with a real Groq API call (not just mocks). Then closed **0.24 (NEW)**: decided (per direct user instruction) that the sync pill's `lastSyncedAt` should exclude non-external-data sources now that classification's fast cadence makes an existing quirk far more visible — see `docs/decisions/14-last-synced-excludes-non-data-sources.md`. Final state: 342 backend tests passing (up from 313), ruff clean, real-Postgres tests added for the new reply endpoint's ordering query, a genuinely aborted Postgres transaction proving the scheduler fix's fallback path, and the new classification job's ordering query, plus a real-browser (Playwright) pass of the alias tooltips and reply-modal flow with zero console errors. Everything else in this document is unchanged from the 2026-09-03/09-05 passes; no other item's status was re-verified false or newly discovered in this update.
...
- [x] **0.25 · NEW (2026-09-12) · Four places still showed fabricated data in a real, logged-in session**
  Raised directly by the user ("remove all mock data on logged in sessions"), not a checklist item — an audit of every `SAMPLE_DATA.*` reference in `index.html` (and a matching audit of the backend routes for canned/templated content) found four places where a real, authenticated session still rendered fabricated content as if it were real: **(a)** the Competitors tab's "Top Voices" and **(b)** "Category Watch — Hair" cards rendered `SAMPLE_DATA.topVoices`/`SAMPLE_DATA.categoryWatch` unconditionally, regardless of `STATE.isDemoMode` — a documented, deliberate gap at Phase 7 time ("no endpoint backs these two cards yet") that nonetheless meant a real session saw fake handles/reach numbers presented as live KOL data; **(c)** `handleAssign()`'s roster dropdown fell back to `SAMPLE_DATA.roster.assignees` (the mockup's original hardcoded Gian/Paul/Boom/Mixi) whenever `STATE.roster` was falsy, with no demo-mode check at all — in practice `initApp()`'s startup gate means this shouldn't trigger in a healthy live session, but nothing stopped it from doing so if the roster fetch state was ever null; **(d)** `GET /api/overview`'s `aiSummaryText` returned one of three canned, pre-written strings on **every single call, unconditionally** — since every caller of that endpoint is by construction an authenticated real session (`get_current_user` is a required dependency), a real user reading real data was shown a fabricated "AI-generated" narrative about a fictional 75-point Clarity Index and a fictional Rappler/Philippine Star coverage week, on every real login. (d) was an explicit, ratified Phase 7 contract decision ("this endpoint may keep returning a canned/templated string for now") — reversed here per the user's direct instruction, not silently.
  **Effort:** M · **Requirement:** — (data integrity/trust, not a P0/P1 acceptance criterion) · **Skip risk:** A tool whose entire purpose is being trusted to report real reputation data shows leadership a fabricated AI summary and fake KOL names on every real login — the single worst kind of gap for exactly this product · CRITICAL
  **Status:** Done. (a)/(b): both cards now check `STATE.isDemoMode` and render an honest "Not built yet" empty state (via the existing `renderStatePanel()` helper) in a real session, keeping the labeled sample content only in demo mode. (c): the roster fallback now only uses `SAMPLE_DATA.roster.assignees` when `STATE.isDemoMode` is true; a live session with no roster shows an empty dropdown, never invented names. (d): `GET /api/overview`'s `aiSummaryText` now returns `null` unconditionally (the dead `_AI_SUMMARIES` constant removed); the mockup renders an honest italic placeholder ("AI summary isn't built yet — nothing to show here for real data") instead of either a blank div or invented text, only when the field is null — demo mode is unaffected, since it never calls this endpoint and gets its sample summaries from `SAMPLE_DATA.aiSummaryVariants` directly. `docs/api-contract.md` updated for the `aiSummaryText` contract change. Verified: updated backend test asserting `aiSummaryText is None`, and a real-browser (Playwright) check simulating a live session (STATE.isDemoMode=false, real-shaped but sparse data) confirming none of the four surfaces render any fabricated content, alongside a regression check that demo mode's sample content is unchanged. 342 tests pass, ruff clean.

- [x] **0.26 · NEW (2026-09-12) · Rename `remedy-pulse-mockup.html` → `index.html`**
  Raised directly by the user ("rename the remedy html to remove mockup as it might be confusing in an actual link when a user logs in") — with real login/auth (5.5), a real API (Phase 7), and 0.25's mock-data removal all now shipped, this file is the production frontend for real sessions, not just a demo artifact, and a filename containing "mockup" was the wrong signal for a real user to see in an address bar or a shared link.
  **Effort:** S · **Requirement:** — · **Skip risk:** A URL or filename visibly reading "mockup" undermines trust in a tool whose whole purpose is being trusted with real reputation data · LOW
  **Status:** Done. `git mv remedy-pulse-mockup.html index.html` (history preserved), then every literal reference to the old filename replaced across ~29 files (docs, backend code comments/docstrings, tests, `docs/decisions/*.md`) — a plain string replace, not a rewrite of the surrounding "mockup" *design-vocabulary* comments that legitimately describe the pre-refactor era (those are developer-facing history, not the filename, and stayed untouched). One real simplification this enabled: `_redirects` (the Cloudflare Pages rewrite that mapped the root URL to the old, non-`index.html` filename) is now unnecessary — Cloudflare Pages serves `index.html` at the root automatically — so the file was removed and `docs/runbook-deploy-free-tier.md` updated to explain why. `README.md`'s repo-layout line and `docs/implementation-checklist.html` (the published checklist artifact) also updated. Verified: full backend suite still green (342 passed, ruff clean — a filename-only change touching Python docstrings/comments, not logic), and the renamed file's inline `<script>` block still parses (`node --check`).

- [x] **0.27 · NEW (2026-09-12) · A malformed `from`/`to` date query param 500'd instead of 400'ing on three routes**
  Found during a fresh sweep for more zero-payment fixes: `GET /api/mentions`, `GET /api/emv`, and `POST /api/exports/{type}` each duplicate their own `_parse_dt()` helper (one per route file, matching this codebase's existing per-route-file convention), and none of the three caught `datetime.fromisoformat()`'s `ValueError` on a malformed value — an uncaught exception, surfaced as a generic 500 rather than a clean validation error. `GET /api/overview` already handles the identical case correctly (`except ValueError: raise ApiError(400, ...)`), so this was an inconsistency between four near-identical helpers, not a design gap needing a decision.
  **Effort:** S · **Requirement:** — · **Skip risk:** A malformed date (a stray typo in a hand-built URL, a client bug, a probing request) reads as a server crash instead of a client error — low real-world frequency (the mockup's own date filters are picker-built, not free text) but a real inconsistency in an otherwise carefully-done API · LOW
  **Status:** Done. All three `_parse_dt()` helpers now catch `ValueError` and raise `ApiError(400, {"error": "'from'/'to' must be ISO-8601 dates"})`, matching `overview.py`'s existing correct pattern exactly. One new test per route confirms a malformed `from` value now returns 400 with that message instead of 500. 345 tests pass (up from 342), ruff clean.
