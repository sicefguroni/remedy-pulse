...
- The EMV tab is the largest set of peso figures anywhere in the product: verified directly in `index.html` — 6 hardcoded article rows (Rappler, Philippine Star, PeopleAsia, When In Manila, Manila Bulletin, ANC; lines 861-931), summing to **Gross ₱2,366,000** and **Net Favorable ₱2,809,000** (lines 842-843). Every one of these rows is static markup with no backing data source of any kind, per-source or otherwise.
